...
def notes():
    ...
    return render_template('notes.html', classes_ecoliers=classes_ecoliers, classes_eleves=classes_eleves)
...
def save_notes():
    ...
    for note_data in notes:
        db.add_note(note_data['student_id'], note_data['student_type'], note_data['classe'], note_data['matiere'], note_data['note'])
        ...
        if student['classe'] == classe:
            ...
            for n in student_notes:
                if n['matiere'] == matiere:
                    ...
                    break
            students.append({'id': student['id'], 'nom': student['nom'], 'prenoms': student['prenoms'], 'note': note_for_matiere})
    return jsonify({'students': students})
...
def export_excel():
    ...
    wb.save(output)
    output.seek(0)
    return send_file(io.BytesIO(output.getvalue()), mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', as_attachment=True, download_name=f'ecole_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
